...
async def view_log_book(filters: Optional[str] = Query(None)):
    ...
    if all_rows:
        for index, row in enumerate(all_rows, start=1):
            ...
            <tr>
                ...
                <td><span class="badge-count">×{req_count}</span></td>
                <td>{profile_cell}</td>
                <td>{post_cell}</td>
            ...
            img_preview = f'<img src="{img_url}" class="card-img" alt="Media"/>' if img_url else '<div class="no-img-box">No Media Asset</div>'
            cards_html += f"""
            <div class="card">
                <div class="card-media">
                    <span class="card-serial">#{index}</span>
                    <span class="card-count">×{req_count} reqs</span>
                    {img_preview}
                </div>
                <div class="card-body">
                    ...
                    <div class="card-actions">
                        {post_cell}
                        {f'<a href="{img_url}" target="_blank" class="btn-sub">Open Media ↗</a>' if img_url else ''}
                    </div>
                </div>
            </div>
            """
    ...
    <head>
        ...
        <style>
            ...
            .metric-title {{ font-size: 11px; text-transform: uppercase; color: var(--text-muted); font-weight: 700; }}
            ...
            .table-wrapper {{ background-color: var(--panel); border: 1px solid var(--border); border-radius: 12px; overflow: auto; }}
            table {{ width: 100%; border-collapse: collapse; text-align: left; font-size: 13px; }}
            th, td {{ padding: 12px 16px; border-bottom: 1px solid var(--border); white-space: nowrap; }}
            th {{ background-color: rgba(15, 23, 42, 0.8); font-weight: 700; color: var(--text-muted); text-transform: uppercase; font-size: 11px; }}
...
# --- PAGE 2: MULTI-PROJECT FOCAL DATASET STUDIO (/dataset-builder) ---
...
async def view_dataset_builder(project_id: Optional[str] = Query(None)):
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Multi-Project Dataset Studio</title>
        <style>
            :root {{
            ...
            .btn {{ background: var(--panel); color: var(--text); border: 1px solid var(--border); padding: 8px 14px; border-radius: 8px; font-weight: 600; font-size: 13px; cursor: pointer; text-decoration: none; }}
            .btn:hover {{ border-color: var(--primary); }}
            .btn-primary {{ background: var(--primary); border-color: var(--primary); color: white; }}
            .btn-success {{ background: var(--success); border-color: var(--success); color: white; }}
            ...
            /* Focal Card Workspace */
            .focal-workspace {{ background: var(--panel); border: 1px solid var(--border); border-radius: 16px; padding: 28px; display: flex; gap: 28px; align-items: center; min-height: 400px; }}
            .media-box {{ flex: 1; height: 360px; background: #000; border-radius: 12px; overflow: hidden; display: flex; align-items: center; justify-content: center; border: 1px solid var(--border); }}
            ...
            .class-picker {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 10px; margin-top: 10px; }}
            .cls-btn {{ padding: 12px; border-radius: 8px; border: 1px solid var(--border); background: #1F2937; color: var(--text); font-weight: 700; font-size: 13px; cursor: pointer; text-align: center; text-transform: capitalize; }}
                ...
                <div class="info-box">
                    <div>
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            ...
                            <span id="classTag" style="font-size:11px; font-weight:800; padding:2px 8px; border-radius:4px; background:var(--border);">CLASS</span>
                        </div>
                        <h2 id="authorName" style="margin: 8px 0 12px 0; font-size: 18px;">Profile Name</h2>
                        <p style="font-size:13px; color:var(--muted); margin:4px 0;"><strong>Post Link:</strong> <a id="postLink" href="#" target="_blank" style="color:var(--primary);">Open Original ↗</a></p>
                    </div>

                    <div>
                        <label style="font-size: 11px; font-weight: 700; color: var(--muted); text-transform: UPPERCASE;">Custom Project Classes:</label>
        ...
        <script>
            ...
            function renderCard() {{
                if (!items || items.length === 0) {{
                    document.getElementById('focalContainer').innerHTML = "<div style='text-align:center; width:100%; color:var(--muted); padding:40px;'>No items in this project yet. Click 'Import Stream Logs' to populate.</div>";
                    return;
                }}

                const item = items[currentIndex];
                document.getElementById('authorName').innerText = item.profileName || "Unknown";
                ...
                document.getElementById('classTag').innerText = (item.customClass || '').toUpperCase();
                ...
                if (item.imageUrl) {{
                    mediaBox.innerHTML = `<img src="${{item.imageUrl}}" alt="Media"/>`;
                }} else {{
                    mediaBox.innerHTML = "<span style='color:var(--muted);'>No Media Asset</span>";
                }}
            ...
            function assignClass(newClass) {{
                ...
                formData.append('post_url', item.postUrl);
                formData.append('new_class', newClass);

                fetch('/api/v1/projects/update-label', {{
                    method: 'PATCH',
                    headers: {{ 'X-Admin-Secret': key }},
                    body: formData
                }})
            ...
            function openNewProjectModal() {{
                ...
                const slug = prompt("Project Slug ID (e.g. crop_disease_v1):");
                if (!slug) return;
                const title = prompt("Project Title (e.g. Crop Disease Classification):");
                const classes = prompt("Custom Class Labels (comma-separated, e.g. healthy,blight,rust):");
            ...
            function importToCurrentProject() {{
                ...
                fetch('/api/v1/projects/import-items', {{
                    method: 'POST',
                    headers: {{ 'X-Admin-Secret': key }},
                    body: formData
                }})
                .then(res => res.json())
                .then(data => {{ alert(data.status ? `Imported ${{data.importedCount}} items.` : data.detail); window.location.reload(); }});
            }}
            ...
            function exportProjectZip() {{
                ...
                .then(blob => {{
                    ...
                    a.href = url;
                    a.download = `${{currentProject.projectId}}.zip`;
