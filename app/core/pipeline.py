...
async def evaluate_analysis_pipeline(image_path: str, metadata: dict) -> dict:
    """
    Standard synchronous-style analysis returning a full JSON dictionary.
    """
    ...
    if img_bgr is None or img_bgr.size == 0:
        return {
            "status": "low_confidence",
            "badge": "🟡 Low Confidence",
            "message": "Gate Check Failed: Unable to decode image.",
            "stages": [{"stage_name": "Stage 0: Input Gate Check", "status": "failed", "execution_time_ms": round((time.time() - t0) * 1000, 2)}],
...
async def evaluate_analysis_pipeline_stream(image_path: str, metadata: dict) -> AsyncGenerator[str, None]:
    ...
    if not image_path or not os.path.exists(image_path):
        yield json.dumps({"event": "stage_complete", "stage_index": 0, "status": "failed", "message": "File not found"}) + "\n"
        return

    img_bgr = cv2.imread(image_path)
    if img_bgr is None or img_bgr.size == 0:
        yield json.dumps({"event": "stage_complete", "stage_index": 0, "status": "failed", "message": "Decode error"}) + "\n"
        return

    # Yield Gatekeeper Pass
    yield json.dumps({
    ...
    # Stage 1 RT-DETR
    t1 = time.time()
    yield json.dumps({"event": "progress", "stage_index": 1, "stage_name": "Stage 1: RT-DETR Region Detection", "progress_percent": 35}) + "\n"
    
    # Run full eval dict internally for stages 1..3
    res = await evaluate_analysis_pipeline(image_path, metadata)
    
    for idx, stg in enumerate(res.get("stages", [])[1:], start=1):
        yield json.dumps({
            "event": "stage_complete",
            "stage_index": idx,
    ...
    yield json.dumps({
        ...
        "confidence_score": res.get("verdict", {}).get("confidence_score", 0.5)
    }) + "\n"